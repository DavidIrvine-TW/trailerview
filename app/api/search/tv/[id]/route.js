...
import { getSearchTV,getTVVideos, TMDB_TOKEN } from "../../../../lib/TMDBEndpoints";
...
export async function GET(request, { params }) {
  ...
  try {
    const response = await fetch(getSearchTV(query, pages), {
      headers: {
    ...
    const tvIds = results.map((item) => item.id);
    const promises = tvIds.map(fetchTVData);
